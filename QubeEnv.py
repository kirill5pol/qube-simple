...
def next_state_fn(dt):
    ...
    def next_state(state, action, dt=dt):
        t = np.linspace(0.0, dt, 2)
        state = np.asarray(state)

        next_state = np.array(odeint(diff_state, state, t, args=(action, dt)))[1, :]
...
class QubeSwingupEnv(gym.Env):
    def __init__(self, frequency=250, **kwargs):
        ...
        self.next_state = next_state_fn(self.frequency)
